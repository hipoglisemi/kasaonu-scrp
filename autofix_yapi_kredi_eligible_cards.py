...
def main():
    ...
    print(f"Successfully parsed {total_parsed} Yapı Kredi campaign updates from the report.")
    ...
    try:
        for camp_id, eligible_cards in updates:
            ...
            if success_count % 20 == 0 or success_count == total_parsed:
                print(f"Applied {success_count}/{total_parsed} Yapı Kredi database updates...")
                
        db.commit()
        print(f"\n🎉 SUCCESS: Successfully updated {success_count} Yapı Kredi campaigns in the database!")
        print("All audited Yapı Kredi campaigns have been marked as is_audited=True and updated with flawless card lists!")
