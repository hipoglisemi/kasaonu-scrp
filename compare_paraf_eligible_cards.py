...
def normalize(name: str) -> str:
    ...
    for word in ["karti", "kartlari", "kart", "logolu", "ozellikli"]:
...
def main():
    ...
    total = len(campaigns)
    print(f"Starting precision mismatch audit for {total} Paraf campaigns...")
    ...
    report_path = "/Users/hipoglisemi/Desktop/kartavantaj/paraf_precision_mismatch_report.md"
    
    with open(report_path, "w", encoding="utf-8") as f:
        f.write(f"# 🔍 Paraf Kampanya Kart Eşleştirme Karşılaştırma Raporu (Hassas Denetim)\n\n")
        f.write(f"Bu denetim raporu, sistemdeki **{total}** aktif Paraf (Halkbank) kampanyasının veri tabanındaki `eligible_cards` sütunu ile metinden kelimesi kelimesine çıkarılan **AI Önerilen Sıralı Listeyi** karşılaştırır.\n\n")
