...
def main():
    ...
    campaigns = db.query(Campaign).join(Campaign.card).filter(
        Card.slug.in_(["crystal", "adios", "play"]),
        Campaign.is_active == True
    ).order_by(Campaign.id.desc()).all()
    
    unique_campaigns = list({c.id: c for c in campaigns}.values())
    total = len(unique_campaigns)
    print(f"Starting precision mismatch audit for {total} Yapı Kredi Niche (Crystal, Adios, Play) campaigns...")
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write(f"# 🔍 Yapı Kredi (Crystal, Adios, Play) Hassas Denetim Karşılaştırma Raporu\n\n")
        f.write(f"Bu denetim raporu, sistemdeki **{total}** aktif Yapı Kredi (Crystal, Adios, Play) kampanyasının veri tabanındaki `eligible_cards` sütunu ile metinden kelimesi kelimesine çıkarılan **AI Önerilen Sıralı Listeyi** karşılaştırır.\n\n")
