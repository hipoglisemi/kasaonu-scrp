...
class EmlakKatilimScraper:
    BASE_URL = "https://www.emlakkatilim.com.tr"
    LIST_URL = "https://www.emlakkatilim.com.tr/tr/bireysel/kampanyalar"
    ...
    def __init__(self):
        ...
        self.bank = self.db.query(Bank).filter(Bank.slug == 'emlak-katilim').first()
        if not self.bank:
            self.bank = Bank(name='Emlak Katılım', slug='emlak-katilim', logo_url='/logos/banks/emlakkatilim.png', is_active=True)
            self.db.add(self.bank)
            self.db.commit()
            
        self.card = self.db.query(Card).filter(Card.slug == 'emlak-katilim-paraf').first()
        if not self.card:
             self.card = Card(
                 bank_id=self.bank.id, 
                 name='Emlak Katılım Paraf', 
                 slug='emlak-katilim-paraf', 
                 logo_url='/logos/cards/emlakkatilimparaf.png',
                 is_active=True
             )
    ...
    def _fetch_campaign_list(self) -> List[Dict[str, Any]]:
        print(f"📄 Fetching Emlak Katılım campaigns from {self.LIST_URL}...")
        campaigns = []
        seen = set()
        ...
        try:
            ...
            for a in links:
                href = a.get('href')
                if href and '/kampanya/' in href.lower() and href != '/tr/bireysel/kampanyalar':
    ...
    def _process_campaign(self, campaign_data):
        ...
        try:
            ...
            body_el = soup.select_one('.o-page__content') or soup.find("body")
            raw_html = str(body_el) if body_el else str(soup)

            detail_img = None
            
            # Look for an image inside the content
            if body_el:
                img_el = body_el.select_one('img')
    ...
    def run(self):
        print("🚀 Starting Emlak Katılım Scraper...")
