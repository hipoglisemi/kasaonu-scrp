...
class GarantiMilesAndSmilesScraper:
    ...
    BASE_URL = 'https://milesandsmilesgarantibbva.com'
    CAMPAIGN_LIST_URL = 'https://milesandsmilesgarantibbva.com/kampanyalar'
    ...
    def __init__(self):
        ...
        with get_db_session() as db:
            ...
            if not card:
                card = Card(bank_id=self.bank.id, name="Garanti Miles&Smiles", slug="garanti-miles-smiles", is_active=True)  # type: ignore # pyre-ignore[16]
    ...
    def _get_or_create_card(self):
        """Get or create Garanti Miles&Smiles card"""
        with get_db_session() as db:
            card = None
    ...
    def run(self):
        """Main execution flow"""
        print("🚀 Garanti Miles&Smiles Scraper - UIkit Edition")
        print("=" * 60)
        ...
        try:
            ...
            if not campaign_urls:
                ...
                with get_db_session() as db:
                     log_scraper_execution(db, "garanti-miles-smiles", "SUCCESS", 0, 0, 0, 0)
                return
            ...
            if int(failed_count or 0) > 0:  # type: ignore # pyre-ignore[58]
                 status = "PARTIAL" if (int(success_count or 0) > 0 or int(skipped_count or 0) > 0) else "FAILED"  # type: ignore # pyre-ignore[58]
                 
            from src.utils.logger_utils import log_scraper_execution  # type: ignore # pyre-ignore[21]
            with get_db_session() as db:
                 log_scraper_execution(
                      db=db,
                      scraper_name="garanti-miles-smiles",
                      status=status,
...
if __name__ == "__main__":
    scraper = GarantiMilesAndSmilesScraper()
    scraper.run()
