...
class GSParaScraper:
    """
    Scraper for GS Para (QNB) campaigns using their public JSON API.
    """

    BASE_URL = "https://www.gspara.com.tr"
    API_URL = "https://www.gspara.com.tr/api/campaign/getall?isArchived=false"
    BANK_NAME = "QNB"
    CARD_NAME = "gspara"
    CARD_SLUG = "gspara"
    ...
    def _fetch_campaigns(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        print(f"   🌐 Fetching campaigns from GS Para API...")
        headers = {
        ...
        try:
            ...
            print(f"   📦 Found {len(data)} campaigns from GS Para API.")
    ...
    def _process_item(self, item: Dict[str, Any]) -> str:
        ...
        campaign_url = f"{self.BASE_URL}/ayricaliklar/{c_path}"
    ...
    def _save_campaign(self, ai_data: Dict[str, Any], url: str, item: Dict[str, Any]) -> str:
        try:
            with get_db_session() as db:
                ...
                # Date parsing with fallback to ai_data dates
                start_date = None
                end_date = None
    ...
    def run(self, limit: Optional[int] = None):
        print(f"🚀 Starting GS Para Scraper...")
