...
class HayatPayScraper:
    BASE_URL = "https://hayatpay.com.tr"
    LIST_URL = "https://hayatpay.com.tr/kampanya"
    ...
    def __init__(self):
        ...
        # Get or create Hayat Pay card
        self.card_hayatpay = self.db.query(Card).filter(Card.slug == 'hayat-pay').first()
        if not self.card_hayatpay:
             self.card_hayatpay = Card(
                 bank_id=self.bank.id, 
                 name='Hayat Pay', 
                 slug='hayat-pay', 
                 logo_url='/logos/cards/hayatpay.webp',
                 is_active=True
             )
             self.db.add(self.card_hayatpay)
             self.db.commit()
             
        # Get or create Hayat Finans card
        self.card_hayatfinans = self.db.query(Card).filter(Card.slug == 'hayat-finans').first()
        if not self.card_hayatfinans:
             self.card_hayatfinans = Card(
                 bank_id=self.bank.id, 
                 name='Hayat Finans', 
                 slug='hayat-finans', 
             ...
             self.db.add(self.card_hayatfinans)
             self.db.commit()

    def _parse_next_f_json(self, html_content: str, key: str) -> Optional[Dict[str, Any]]:
        """Extract JSON structure pushed in Next.js Server Components chunk."""
        # Find key in format key\": or key\":{ or key":{
        ...
        start_idx = match.end()
        raw_chunk = html_content[start_idx:start_idx+150000] # Large buffer for campaigns JSON
        
        # Replace escaped quotes and backslashes
        clean_chunk = raw_chunk.replace('\\"', '"').replace('\\\\', '\\')
        
        # Balance braces to find valid JSON block end
        brace_count = 0
        json_str = ""
        ...
        try:
            ...
            print("   ⚠️ JSON parse error on Next.js chunk:", e)
            return None

    def _fetch_campaign_list(self) -> List[Dict[str, Any]]:
        print(f"📄 Fetching Hayat Pay & Hayat Finans campaigns from {self.LIST_URL}...")
        campaigns = []
        
        try:
            resp = self.session.get(self.LIST_URL, timeout=30)
            data = self._parse_next_f_json(resp.text, 'initialCampaignsData')
            if not data:
                print("   ❌ Failed to extract initialCampaignsData from page.")
                return []
                
            # Hayat Pay tab campaigns
            hayat_pay_items = data.get('hayatPay', {}).get('items', [])
            print(f"   Found {len(hayat_pay_items)} campaigns under Hayat Pay tab.")
            for item in hayat_pay_items:
                seo_url = item.get('seoUrl')
                if seo_url:
                    campaigns.append({
                        "url": urljoin(self.BASE_URL, f"/kampanya/{seo_url}"),
                        "title": item.get('name'),
                        "image": item.get('images', [{}])[0].get('path') if item.get('images') else None,
                        "card_type": "hayat-pay"
                    })
                    
            # Hayat Finans tab campaigns
            bank_items = data.get('bankCampaigns', {}).get('items', [])
            print(f"   Found {len(bank_items)} campaigns under Hayat Finans tab.")
        ...
        print(f"   ✅ Total unique campaigns parsed from JSON: {len(campaigns)}")
        return campaigns

    def _process_campaign(self, campaign_data: Dict[str, Any]) -> str:
        url = campaign_data['url']
        card_type = campaign_data['card_type']
        
        card_obj = self.card_hayatpay if card_type == "hayat-pay" else self.card_hayatfinans
        card_id = card_obj.id
        ...
        try:
            ...
            # Content is stored inside class 'content'
    ...
    def run(self):
        print("🚀 Starting Hayat Pay Scraper...")
