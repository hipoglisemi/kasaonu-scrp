...
class SipayScraper:
    BASE_URL = "https://sipay.com.tr/kampanya/"
    ...
    def __init__(self):
        ...
        self.bank = self.db.query(Bank).filter(Bank.slug == 'sipay').first()
        if not self.bank:
            self.bank = Bank(name='Sipay', slug='sipay', logo_url='/logos/sipay.png', is_active=True)
            self.db.add(self.bank)
            self.db.commit()
            
        self.card = self.db.query(Card).filter(Card.slug == 'sipay-kart').first()
        if not self.card:
             self.card = Card(bank_id=self.bank.id, name='Sipay Kart', slug='sipay-kart', is_active=True)
    ...
    def _fetch_campaign_list(self) -> List[Dict[str, str]]:
        print(f"📄 Fetching Sipay campaigns from HTML...")
        campaigns = []
        try:
            ...
            for a in links:
                href = a.get('href', '')
                if 'kampanya/' in href and href != self.BASE_URL and href != '/kampanya/':
    ...
    def _process_campaign(self, campaign_data):
        ...
        try:
            ...
            if existing and existing.is_active and existing.is_approved:
                print(f"   ⏭️ Skipped (Already exists and active): {existing.title[:40] if existing.title else url}")
                return "skipped"
        except Exception as e:
            pass
        ...
        try:
            ...
            if main:
                for element in main.find_all(['p', 'li']):
                    ...
                    if len(text) > 15 and text.lower() not in ["sanal pos", "sipay cüzdan", "online ödeme çözümleri", "para transferleri", "giriş yap", "kurumsal", "bireysel"]:
