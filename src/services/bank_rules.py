...
- TERMINOLOGY: "Worldpuan" is the currency.
    ...
    - 🚨 **KAYNAK KONTROLÜ (SAFE ZONE)**: Yapı Kredi (World) metinlerinde kart listeleri genellikle ayrı bir başlık altında listelenmez. Genellikle "Kampanya Koşulları" veya "Kampanyaya Dahil Harcamalar" başlığı altındaki maddelerden birinde veya genel metin içinde tek bir cümle halinde geçer (Örn: "Yapı Kredi bireysel kredi kartları ve bireysel TLcard’lar dahildir; Vakıfbank ve Albaraka..."). Bu yüzden özel bir başlık aramak yerine tüm metni ("Kampanya Koşulları" altındaki tüm maddeleri) tarayarak kartların dahil/hariç olduğunu belirten cümleyi izole et ve oradaki kartları çıkar.
    ...
    - 🚨 **BAŞLIK KURALI (CRITICAL)**: "Kampanyaya dahil/dâhil olmayan kartlar:" veya "Kampanyaya katılamayan kartlar:" başlığı altında veya hariç tutulma cümlesinde geçen hiçbir kartı `cards` listesine EKLEME. 🚨 **İSTİSNA**: Bir alt varyasyonun (Örn: "Play Genç", "World Eko") hariç tutulması, ana kartın (Örn: "Play", "Worldcard") hariç tutulduğu anlamına GELMEZ. Eğer metinde "Play kartlar dahildir, Play Genç dahil değildir" diyorsa, "Play" kartını listeye ekle ama "Play Genç"i ekleme.
